...
const CommuteComponent: React.FC<CommuteComponentProps> = ({ departures }) => {
  ...
  useEffect(() => {
    const filtered = departures.filter(
      (dep) =>
        parseInt(formatTimeAndCountMinutesLeft(dep.time).split(" ")[0]) > 0,
    );
        ...
        <Table className="table" aria-label="simple table">
          <TableHead>
            <TableRow className="table-row title-row">
              <TableCell className="table-cell table-cell-title">
                Linje
              </TableCell>
              <TableCell className="table-cell table-cell-title destination">
                Destination
              </TableCell>
          ...
          <TableBody>
            {[0, 1, 2, 3, 4, 5].map((index) => (
              <TableRow
                ...
                sx={{
                  backgroundColor:
                    index % 2 === 0 ? "white" : "rgba(100, 250, 113, 0.7)",
                  color: index % 2 === 0 ? "#000000" : "#FFFFFF",
                }}
                key={index}
              >
                <TableCell className="table-cell">
                  {filteredDepartures[index]
                    ? fixLineName(filteredDepartures[index].name)
                    : "---"}
                </TableCell>
                <TableCell className="table-cell">
                  {filteredDepartures[index]
                    ? filteredDepartures[index].direction
                    : "---"}
                </TableCell>
                <TableCell className="table-cell">
                  {filteredDepartures[index]
                    ? formatTimeAndCountMinutesLeft(
                        filteredDepartures[index].time,
                      )
                    : "---"}
                </TableCell>
              </TableRow>
