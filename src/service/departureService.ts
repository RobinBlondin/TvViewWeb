...
export const getDeparturesByLine = async (
  line: string,
): Promise<DepartureModel[]> => {
  return apiClient.get(`${API_BASE_URL}/line/${line}`).then((res) => res.data);
};

export const getDeparturesByDirection = async (
  direction: string,
): Promise<DepartureModel[]> => {
  return apiClient
    .get(`${API_BASE_URL}/direction/${direction}`)
    .then((res) => res.data);
};

export const getDeparturesByLineAndDirection = async (
  line: string,
  direction: string,
): Promise<DepartureModel[]> => {
  return apiClient
    .get(`${API_BASE_URL}/line-direction/${line}/${direction}`)
    .then((res) => res.data);
};
